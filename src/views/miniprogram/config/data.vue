<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="homeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="appid" align="center" prop="appid" :show-overflow-tooltip="true"/>
      <el-table-column label="图片" align="center" prop="img">
        <template slot-scope="scope">
          <el-image :src="getImage(scope.row.img)" style="width:150px"></el-image>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    created() {
      this.queryParams.appid = this.$route.params && this.$route.params.appid;
      this.getList();
    },
    methods: {
      /** 查询轮播图列表 */
      ...
      submitForm() {
        this.$refs["form"].validate(valid => {
          if (this.form.appid === undefined) {
            this.$modal.notifyError("appid不能为空");
          } else if (this.form.img === undefined) {
            this.$modal.notifyError("图片不能为空");
          }
          if (valid) {
